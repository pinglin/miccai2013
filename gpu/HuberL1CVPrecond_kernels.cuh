                           ...
                           size_t const height)
{
    ...
    if (x < width && y < height && z < num_disp_laysers)
    {
        ...
        {
        case LeftRefImage:
            if(x >= win_r && x < width-win_r && y >= max_disp+win_r && y < height-win_r)
            {
                for(int win_x = x-win_r; win_x <= x+win_r; win_x++)
                    for(int win_y = y-win_r; win_y <= y+win_r; win_y++)
                    {
                        ...
                        double target_gray = tex2D(target_img_tex, win_x, win_y-disp);
            ...
            if(x >= win_r && x < width-win_r && y >= win_r && y < height-max_disp-win_r)
            {
                for(int win_x = x-win_r; win_x <= x+win_r; win_x++)
                    for(int win_y = y-win_r; win_y <= y+win_r; win_y++)
                    {
                        ...
                        float target_gray = tex2D(target_img_tex, win_x, win_y+disp);
                                    ...
                                    size_t const height)
{
    ...
    {
    case LeftRefImage:
        if(x >= win_r && x < width-win_r-1 && y >= max_disp+win_r && y < height-win_r-1)
        {
    ...
    case LeftRefImage:
        if(x >= win_r && x < width-win_r && y >= max_disp+win_r && y < height-win_r)
        {
            ...
            for(uint8_t z = lower_bound; z <= upper_bound; z++)
            {
                ...
                Eaux = 0.5f*(1.0f/aux_theta)*pow((tex2D(primal_tex, x, y) - aux_noramlised),2.0f) + lambda*tex3D(cost_volume_tex, x, y, z);
                if(Eaux < Eaux_min)
                {
                    Eaux_min = Eaux;
